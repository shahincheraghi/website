...
@section('title')
    | {{trans('langPanel.BusinessProcessList')}}
@endsection
@section('css')
                            ...
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.listBusinessProcess')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <section id="data-list-view" class="data-list-view-header">
                    ...
                    <div class="row justify-content-end pr-5">
                        <div>
                            ...
                            <a class="btn btn-outline-primary" href="{{route('Admin.BusinessProcess.create')}}">
                                جدید
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tbody>
                            @php $i=1; @endphp
                            @forelse($BusinessProcess as $item)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$item->title}}</td>
                                    <td>{{$item->link}}</td>
                                    ...
                                    <td>
                                        <a href="{{route('Admin.BusinessProcess.edit',$item->id)}}"
                                           type="button"
                                           class="btn btn-icon  rounded-circle btn-warning mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>
                                        <a onclick="deleteRecord('{{route('Admin.BusinessProcess.destroy',$item->id)}}')"
                                           type="button"
