...
@section('title')| {{trans('langPanel.ColorList')}} @endsection
...
@section('content')
                            ...
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.listColor')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <table class="table data-list-view">
                            ...
                            @forelse($Color as $item)
                                <tr>
                                    ...
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->sort}}</td>
                                    <td><img src="/File/Color/{{$item->img}}" alt="image" width="150px"
                                             height="100px"></td>
                                    <td>
                                        @if($item->status == 0)
                                            <span>غیرفعال</span>
                                    ...
                                    <td>
                                        <a onclick="showEditColor({{$item->id}})"
                                           type="button"
                                           class="btn btn-icon rounded-circle btn-info mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>

                                        <a onclick="deleteRecord('{{route('Admin.ColorController.destroy',$item->id)}}')"
                                           type="button"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="colorModalLabel">ثبت رنگ</h5>
    ...
    <div class="modal fade" id="colorModalIUpdate" tabindex="-1" aria-labelledby="colorModalIUpdate"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="colorModalLabel">ویرایش رنگ</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('script')
    ...
    {{--    =========show data edit =========--}}
    <script type="text/javascript">
        function showEditColor(id) {
            $.ajax({
                url: "/Admins/Color/edit",
                type: "POST",
                data: {id: id},
                success: function (response) {
                    ...
                    document.getElementById('imageSrc').src = "/File/Color/" + response.data.img;
