...
@section('title')| {{trans('langPanel.ProblemEventList')}} @endsection
...
@section('content')
                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">{{trans('langPanel.ProblemEvent')}}</a>
                                    </li>
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.listProblemEvent')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <section id="data-list-view" class="data-list-view-header">
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tbody>
                            @php $i=1 @endphp
                            @forelse($ProblemEvent as $item)
                                <tr>
                                    <td>{{$i++}}</td`>
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->sort}}</td>
                                    <td><img src="/File/ProblemEvent/{{$item->img}}" alt="image" width="150px"
                                             height="100px"></td>
                                    <td>
                                        @if($item->status == 0)
                                            <span>غیرفعال</span>
                                    ...
                                    <td>
                                        <a onclick="showEditProblemEvent({{$item->id}})"
                                           type="button"
                                           class="btn btn-icon rounded-circle btn-info mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>

                                        <a onclick="deleteRecord('{{route('Admin.ProblemEventController.destroy',$item->id)}}')"
                                           type="button"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="colorModalLabel">ثبت علت خرابی جدید</h5>
    ...
    <!-- Modal Update-->
    <div class="modal fade" id="colorModalIUpdate" tabindex="-1" aria-labelledby="colorModalIUpdate"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="colorModalLabel">ویرایش علت خرابی</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('script')
    ...
    {{--    =========show data edit =========--}}
    <script type="text/javascript">
        function showEditProblemEvent(id) {
            $.ajax({
                url: "/Admins/ProblemEvent/edit",
                type: "POST",
                data: {id: id},
                success: function (response) {
                    ...
                    document.getElementById('imageSrc').src = "/File/ProblemEvent/" + response.data.img;
                    $('#statusUpdate').val(response.data.status).change();
                    $('#colorModalIUpdate').modal('show');
                },
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#categoryFormUpdateForm").on("submit", function (e) {
                e.preventDefault();
                $.ajax({
                    url: "/Admins/ProblemEvent/update",
                    type: "POST",
                    data: new FormData(this),
                    dataType: "JSON",
                    contentType: false,
                    ...
                    success: function (data) {
                        ...
                        Swal.fire(
                            ...
                            if (result.isConfirmed) {
                                // Reset form fields
                                $("#categoryFormUpdateForm")[0].reset();
                                // Hide modal
                                $("#colorModalIUpdate").modal("hide");
                                // Refresh category list
                                $("#categoryList").DataTable().ajax.reload();
