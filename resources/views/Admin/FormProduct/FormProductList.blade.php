...
@section('title')| {{trans('langPanel.FormProductList')}} @endsection
...
@section('content')
                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">{{trans('langPanel.FormProduct')}}</a>
                                    </li>
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.listFormProduct')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tr>
                                ...
                                <th>{{trans('langPanel.category_id')}}</th>
                            ...
                            <tbody>
                            @php $i=1 @endphp
                            @forelse($FormProduct as $item)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->category_id}}</td>
                                    <td>{{$item->sort}}</td>
                                    <td><img src="/File/FormProduct/{{$item->img}}" alt="image" width="150px"
                                             height="100px"></td>
                                    <td>
                                        @if($item->status == 0)
                                            <span>غیرفعال</span>
                                    ...
                                    <td>
                                        <a onclick="showEditFormProduct({{$item->id}})"
                                           type="button"
                                           class="btn btn-icon rounded-circle btn-info mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>

                                        <a onclick="deleteRecord('{{route('Admin.FormProductController.destroy',$item->id)}}')"
                                           type="button"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="productForm" class="productForm" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="status">{{trans('langPanel.category_id')}}</label>
                            <select class="select2 form-control"  name="category_id"
                                    id="category_id">
                                <option ></option>
                                @foreach($FormCategory as $data)

                                    <option value="{{$data->id}}">{{$data->name}}</option>
                                @endforeach
                            </select>
                            <span class="invalid-feedback" role="alert" id="category_idError"></span>
                        </div>
    ...
    <!-- Modal Update-->
    <div class="modal fade" id="productModalIUpdate" tabindex="-1" aria-labelledby="productModalIUpdate"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form id="productFormUpdateForm" class="productFormUpdateForm" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="status">{{trans('langPanel.category_id')}}</label>
                            <select class="select2 form-control"  name="category_id"
                                    id="category_idUpdate">
                                <option ></option>
                                @foreach($FormCategory as $data)
                                    <option value="{{$data->id}}">{{$data->name}}</option>
                                @endforeach
                            </select>
                            <span class="invalid-feedback" role="alert" id="category_idUpdateError"></span>
                        </div>
...
@section('script')
    ...
    {{--    =========show data edit =========--}}
    <script type="text/javascript">
        function showEditFormProduct(id) {
            $.ajax({
                url: "/Admins/FormProduct/edit",
                type: "POST",
                data: {id: id},
                success: function (response) {
                    ...
                    document.getElementById('imageSrc').src = "/File/FormProduct/" + response.data.img;
                    $('#statusUpdate').val(response.data.status).change();
                    $('#category_idUpdate').val(response.data.category_id).trigger('change');
                    $('#productModalIUpdate').modal('show');
                },
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            // Submit form data using Ajax
            $("#productFormUpdateForm").on("submit", function (e) {
                e.preventDefault();
                $.ajax({
                    url: "/Admins/FormProduct/update",
                    type: "POST",
                    data: new FormData(this),
                    dataType: "JSON",
                    contentType: false,
                    ...
                    success: function (data) {
                        ...
                        Swal.fire(
                            ...
                            .then((result) => {
                                if (result.isConfirmed) {
                                    // Reset form fields
                                    $("#productFormUpdateForm")[0].reset();
                                    // Hide modal
                                    $("#productModalIUpdate").modal("hide");
                                    // Refresh category list
                                    $("#productList").DataTable().ajax.reload();
                    ...
                    error: function (xhr, status, error) {
                        ...
                        if (errors.category_id) {
                            $("#category_idUpdate").addClass("is-invalid");
                            $("#category_idUpdateError").text(errors.img[0]);
                        }
                    }
                });
            });
