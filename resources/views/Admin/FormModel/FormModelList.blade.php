...
@section('title')| {{trans('langPanel.FormModelList')}} @endsection
...
@section('content')
                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">{{trans('langPanel.FormModel')}}</a>
                                    </li>
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.listFormModel')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tr>
                                ...
                                <th>{{trans('langPanel.product_id')}}</th>
                            ...
                            <tbody>
                            @php $i=1 @endphp
                            @forelse($FormModel as $item)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->product_id}}</td>
                                    <td>{{$item->sort}}</td>
                                    <td><img src="/File/FormModel/{{$item->img}}" alt="image" width="150px"
                                             height="100px"></td>
                                    <td>
                                        @if($item->status == 0)
                                            <span>غیرفعال</span>
                                    ...
                                    <td>
                                        <a onclick="showEditFormModel({{$item->id}})"
                                           type="button"
                                           class="btn btn-icon rounded-circle btn-info mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>

                                        <a onclick="deleteRecord('{{route('Admin.FormModelController.destroy',$item->id)}}')"
                                           type="button"
    ...
    <!-- Modal Create-->
    <div class="modal fade" id="modelModal" tabindex="-1" aria-labelledby="modelModal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modelModalLabel">ثبت مدل فرم</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="modelForm" class="modelForm" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="status">{{trans('langPanel.product_id')}}</label>
                            <select class="select2 form-control"  name="product_id"
                                    id="product_id">
                                <option ></option>
                                @foreach($FormProduct as $data)
                                    <option value="{{$data->id}}">{{$data->name}}</option>
                                @endforeach
                            </select>
                            <span class="invalid-feedback" role="alert" id="product_idError"></span>
                        </div>
    ...
    <!-- Modal Update-->
    <div class="modal fade" id="modelModalIUpdate" tabindex="-1" aria-labelledby="modelModalIUpdate"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modelModalLabel">ویرایش مدل فرم</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="modelFormUpdateForm" class="modelFormUpdateForm" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <span class="invalid-feedback" role="alert" id="product_idUpdateError"></span>
                        </div>
...
@section('script')
    ...
    {{--    =========show data edit =========--}}
    <script type="text/javascript">
        function showEditFormModel(id) {
            $.ajax({
                url: "/Admins/FormModel/edit",
                type: "POST",
                data: {id: id},
                success: function (response) {
                    ...
                    document.getElementById('imageSrc').src = "/File/FormModel/" + response.data.img;
                    $('#statusUpdate').val(response.data.status).change();
                    $('#product_idUpdate').val(response.data.product_id).trigger('change');
                    $('#modelModalIUpdate').modal('show');
                },
                ...
                $.ajax({
                    ...
                    error: function (xhr, status, error) {
                        ...
                        if (errors.product_id) {
                            ...
                            $("#product_idError").text(errors.product_id[0]);
                        }
                        if (errors.status) {
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            // Submit form data using Ajax
            $("#modelFormUpdateForm").on("submit", function (e) {
                e.preventDefault();
                $.ajax({
                    url: "/Admins/FormModel/update",
                    type: "POST",
                    data: new FormData(this),
                    dataType: "JSON",
                    contentType: false,
                    ...
                    success: function (data) {
                        ...
                        Swal.fire(
                            ...
                            .then((result) => {
                                if (result.isConfirmed) {
                                    // Reset form fields
                                    $("#modelFormUpdateForm")[0].reset();
                                    // Hide modal
                                    $("#modelModalIUpdate").modal("hide");
                                    // Refresh product list
                                    $("#productList").DataTable().ajax.reload();
                    ...
                    error: function (xhr, status, error) {
                        ...
                        if (errors.product_id) {
                            $("#product_idUpdate").addClass("is-invalid");
                            $("#product_idUpdateError").text(errors.img[0]);
                        }
                    }
                });
            });
