...
@section('title')
    | {{trans('langPanel.HomeContentInfo')}}
@endsection
@section('css')
                            ...
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.listHomeContentInfo')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <section id="data-list-view" class="data-list-view-header">
                    ...
                    <div class="row justify-content-end pr-5">
                        <div>
                            ...
                            <a class="btn btn-outline-primary" href="{{route('Admin.HomeContentInfo.create')}}">
                                جدید
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tr>
                                ...
                                <th>{{trans('langPanel.keyHomeContent')}}</th>
                            ...
                            @forelse($HomeContentInfo as $item)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$item->subTitle}}</td>
                                    <td>{{$item->title}}</td>
                                    <td>{{$item->description}}</td>
                                    <td>{{$item->keyHomeContent}}</td>
                                    <td>
                                        @if($item->status == 3)
                                            <span>غیرفعال</span>
                                    ...
                                    <td>
                                        <a href="{{route('Admin.HomeContentInfo.edit',$item->id)}}"
                                           type="button"
                                           class="btn btn-icon  rounded-circle btn-warning mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>
                                        <a onclick="deleteRecord('{{route('Admin.HomeContentInfo.destroy',$item->id)}}')"
                                           type="button"
