...
@section('title')| {{trans('langPanel.ProblemList')}} @endsection
...
@section('content')
                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">{{trans('langPanel.Problem')}}</a>
                                    </li>
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.listProblem')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <section id="data-list-view" class="data-list-view-header">
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tbody>
                            @php $i=1 @endphp
                            @forelse($Problem as $item)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->sort}}</td>
                                    <td><img src="/File/Problem/{{$item->img}}" alt="image" width="150px"
                                             height="100px"></td>
                                    <td>
                                        @if($item->status == 0)
                                            <span>غیرفعال</span>
                                    ...
                                    <td>
                                        <a onclick="showEditProblem({{$item->id}})"
                                           type="button"
                                           class="btn btn-icon rounded-circle btn-info mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>

                                        <a onclick="deleteRecord('{{route('Admin.ProblemController.destroy',$item->id)}}')"
                                           type="button"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="colorModalLabel">ثبت مشکلات</h5>
    ...
    <div class="modal fade" id="colorModalIUpdate" tabindex="-1" aria-labelledby="colorModalIUpdate"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="colorModalLabel">ویرایش مشکلات</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
        ...
        function showEditProblem(id) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    document.getElementById('imageSrc').src = "/File/Problem/" + response.data.img;
