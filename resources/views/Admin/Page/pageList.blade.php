                            ...
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active"><a
                                            href="#">{{trans('langPanel.pageList')}}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
            ...
            <div class="content-body">
                ...
                <section id="data-list-view" class="data-list-view-header">
                    ...
                    <div class="row justify-content-end pr-5">
                        <div>
                            <a class="btn btn-outline-primary" href="{{route('Admin.pages.create')}}">جدید</a>
                        </div>
                    </div>
                    @include('Layouts.msg')
                        ...
                        <table class="table data-list-view">
                            ...
                            @foreach($data['Pages'] as $value)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$value->title}}</td>
                                    <td style="direction: ltr !important;">{{$data['settings']['domainSite']??''}}/pages/id={{$value->id}}/{{$value->slug}}</td>
                                    ...
                                    <td>
                                        <a href="{{route('Admin.pages.edit',$value->id)}}" type="button"
                                           class="btn btn-icon  rounded-circle btn-warning mr-1 mb-1 waves-effect waves-light"><i
                                                class="feather icon-edit"></i></a>
                                        <a onclick="deleteRecord('{{route('Admin.pages.destroy',$value->id)}}')"
                                           type="button"
